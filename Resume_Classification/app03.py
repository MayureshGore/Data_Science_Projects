...
# FUNCTIONS# Update the extract_skills function to accept resume_text as input
def extract_skills(resume_text, skills_list):
    ...
    for token in tokens: # check for one-grams (example: python)
        if token.lower() in skills_list:  # Check against the skills_list parameter
            skillset.append(token)

    for token in noun_chunks: # check for bi-grams and tri-grams (example: machine learning)
        token = token.text.lower().strip()
        if token in skills_list:  # Check against the skills_list parameter
            skillset.append(token)
...
def getText(filename):
    ...
    if filename.type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
        ...
        with pdfplumber.open(filename) as pdf_file:
            pdoc = PdfReader(filename)
            number_of_pages = len(pdoc.pages)  # Updated line to use len(reader.pages)
            page = pdoc.pages[0]
...
for doc_file in upload_file:
    if doc_file is not None:
        ...
        skills.append(extract_skills(extText, skills))  # Pass skills list to extract_skills function
